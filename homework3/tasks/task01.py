...
def new_cache(
    times,
):  # Параметризованный декоратор, где параметр times говорит сколько раз нужно закэшировать значение, если times=3, значит 3 раза
    cache_value = (
        dict()
    )  # Уже не глобальная переменная, а локальная, внутри декоратора - словарь, пока пустой
    counter = 1  # счетчик количества обращений, если обращаются значит это уже "1", поэтому всегда начинается с "1"
    ...
    def cache_func(
        ...
        def the_wrapper(
            ...
            nonlocal cache_value, counter  # неглобальные переменные, объявленные ранее
            new_value = (
                a,
                b,
            )  # кортуж с аргументами искомойфункции func, объявленной ранее, так просто, для удобства восприятия
            if (
                counter >= times
                ...
                counter = 1  # если же это уже 4-е обращение, сбрасываем счетчик обращений на исходную counter == 1
            if (
...
if __name__ == "__main__":
    ...
    print(
        val_3 is val_4
    )  # кэшируются ли 3й и 4й вызов функции, False, 4й уже не из кэша, считается заново
